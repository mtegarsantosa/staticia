  ...
  return new Promise((resolve,reject) => {
    recDir(path.join(additionalPath), function (err, files) {
      ...
      if (err) reject(err)
        ...
        planjs.convertToObject(arr)
        fs.writeFile(path.join(global.build_dir, 'data.js'), `var data = ${JSON.stringify(planjs.results)}`, function (err) {
          if (err) throw err
        })
        resolve()
      }
